...
void INS_Task(void *pvParameters)
{
        ...
        for (;;)
        {
                ...
                if ((count % 1) == 0)
                {
                        ...
                        INS.Accel[INS_X] = IMU_Get_Accel(&himu1)[0];
                        INS.Accel[INS_Y] = IMU_Get_Accel(&himu1)[1];
                        INS.Accel[INS_Z] = IMU_Get_Accel(&himu1)[2];
                        INS.Gyro[INS_X]  = IMU_Get_Gyro(&himu1)[0];
                        INS.Gyro[INS_Y]  = IMU_Get_Gyro(&himu1)[1];
                        INS.Gyro[INS_Z]  = IMU_Get_Gyro(&himu1)[2];
                        ...
                        INS.atanxz = -atan2f(INS.Accel[INS_X], INS.Accel[INS_Z]) * 180 / PI;
                        INS.atanyz = atan2f(INS.Accel[INS_Y], INS.Accel[INS_Z]) * 180 / PI;
                ...
                vTaskDelay(INS_TASK_PERIOD); // 1ms周期, 让出CPU给低优先级任务
        }
}
